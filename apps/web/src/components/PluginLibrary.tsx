...
function sectionTitle(value: string, unknownLabel = "Unknown"): string {
  const n = value.trim();
  ...
  for (const entry of filteredPluginEntries) {
    ...
    if (existing) {
      ...
      marketplaceSectionsByPath.set(entry.marketplacePath, {
        title: sectionTitle(entry.marketplaceName, t("Unknown")),
        entries: [entry],
      });
    }
  }
        ...
        <div className="min-h-0 flex-1 overflow-y-auto">
          ...
          <div className="px-6 py-10 text-center">
            <h1 className="text-[28px] font-semibold text-foreground">
              {t("Make {provider} work your way", { provider: providerLabel })}
            </h1>
          </div>
              ...
              (pluginsQuery.data?.marketplaceLoadErrors.length ?? 0) > 0 ? (
                <InlineWarning>
                  {pluginsQuery.data?.marketplaceLoadErrors
                    .map(
                      (err) => `${sectionTitle(err.marketplacePath, t("Unknown"))}: ${err.message}`,
                    )
                    .join(" • ")}
                </InlineWarning>
              ) : null}
            ...
            {selectedTab === "plugins" ? (
              <>
                {!canListPlugins ? (
                  <div className="mx-auto max-w-2xl">
                    <EmptyPanel
                      title={t("Plugins unavailable for {provider}", { provider: providerLabel })}
              ...
              <>
                {!canListSkills ? (
                  <div className="mx-auto max-w-2xl">
                    <EmptyPanel
                      title={t("Skills unavailable for {provider}", { provider: providerLabel })}
